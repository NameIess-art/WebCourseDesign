  ...
  <section class="feature-layout">
    ...
    <section class="section-card">
      ...
      <div class="stack-list">
        <article v-for="address in addresses.content" :key="address.id" class="row-card">
          <div class="row-main">
    ...
    <section class="section-card">
      ...
      <div class="product-grid">
        <article v-for="coupon in coupons.content" :key="coupon.id" class="stat-card">
    ...
    <section id="seckill" class="section-card">
      ...
      <div class="product-grid">
        <article v-for="event in seckillEvents.content" :key="event.id" class="product-card">
    ...
    <section class="section-card">
      ...
      <div class="dual-grid">
        <div class="stack-list">
          <h3>收藏商品</h3>
          <article v-for="favorite in favorites.content" :key="favorite.id" class="row-card">
          ...
          <p v-if="!favorites.content.length" class="muted">暂无收藏商品。</p>
        </div>
        <div class="stack-list">
          <h3>积分流水</h3>
          <article v-for="record in pointRecords.content" :key="record.id" class="row-card">
            <div class="row-main">
              <strong>{{ formatStatus(record.type) }} {{ record.points }}</strong>
              <span>{{ record.description }}</span>
            </div>
          </article>
          <p v-if="!pointRecords.content.length" class="muted">暂无积分流水。</p>
        </div>
      </div>
      <div class="stack-list">
        <h3>站内消息</h3>
        <article v-for="message in messages.content" :key="message.id" class="row-card">
          <div class="row-main">
        ...
        <p v-if="!messages.content.length" class="muted">暂无站内消息。</p>
      </div>
    </section>
  </section>
</template>
...
const favorites = ref({ content: [], page: 0, totalPages: 0 })
const messages = ref({ content: [], page: 0, totalPages: 0 })
const pointRecords = ref({ content: [], page: 0, totalPages: 0 })
const seckillEvents = ref({ content: [], page: 0, totalPages: 0 })
